...
<details>
<summary>Screenshot — coming soon</summary>

<!--
TODO (capture target — pass to whoever takes the screenshot):
- Input text: "Every scientist knows this proves our point." (cascade-pair fixture)
- State: post-Analyze, pre-resolution
- Frame must show: annotated text region with one solid-bordered red highlight
  and one dashed amber highlight; one ConfirmedCard ("Confirmed — Ad Populum")
  and one PossiblyCard ("Possibly — Circular Reasoning") with the
  IF LEGITIMATE / IF FALLACY two-column panel visible
- Viewport: 1280x900, dark background (#0f1117), no scroll required
- Save to: docs/screenshots/demo.png (PNG, target ≤500 KB)
-->

A capture of the analyzer with one confirmed and one possibly span will land here once recorded.
